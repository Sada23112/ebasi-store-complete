        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6 lg:gap-8 mb-16 animate-in fade-in slide-in-from-bottom-8 duration-700 animate-stagger-2">
          {products.map((product) => (
            <Link href={`/product/${product.slug}`} key={product.id} className="block group">
              <Card className="overflow-hidden rounded-2xl border border-transparent hover:border-primary/20 premium-shadow hover:premium-shadow-hover transition-all duration-500 ease-out hover:-translate-y-1.5 bg-card/50 backdrop-blur-sm h-full">
                ...
                <CardContent className="p-3 sm:p-5 flex flex-col justify-between h-[calc(100%-192px)] sm:h-[calc(100%-320px)]">
                  ...
                  <div className={cn(buttonVariants({ size: "sm" }), "w-full mt-4")}>
                    View Details
                  </div>
                </CardContent>
              </Card>
