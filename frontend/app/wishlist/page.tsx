...
export default function WishlistPage() {
  ...
  const getWhatsAppUrl = (product: WishlistItem) => {
    ...
    const message = `Hi! I'm interested in ordering from my Wishlist:\n\n*${product.name}*\nPrice: ₹${product.price}\n\nProduct Link: ${productUrl}`
                    ...
                    <Card key={item.id} className="overflow-hidden rounded-2xl border border-border shadow-sm hover:shadow-md transition-all flex flex-col h-full">
                      <div className="relative bg-muted h-64 overflow-hidden rounded-t-2xl group">
                        ...
                        <Button
                          variant="destructive"
                          size="icon"
                          className="absolute top-3 right-3 h-9 w-9 rounded-full shadow-md opacity-90 hover:opacity-100"
                          onClick={(e) => {
                            e.preventDefault()
                            e.stopPropagation()
                            toggle(item)
                        ...
                        {item.stock_status === "out_of_stock" && (
                          <Badge variant="secondary" className="absolute top-3 left-3 bg-red-600 text-white border-none">
                            Out of Stock
                          </Badge>
                        )}
                      </div>
                        ...
                        <div>
                          ...
                          <div className="flex items-baseline gap-2 mt-2">
                            ...
                            {item.compare_price && parseFloat(String(item.compare_price)) > parseFloat(String(item.price)) && (
                              <span className="text-sm text-muted-foreground line-through">₹{item.compare_price}</span>
                            )}
                          </div>
                        </div>
